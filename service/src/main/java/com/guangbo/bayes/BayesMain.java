...
public class BayesMain {
    ...
    public void train() {
        String[] Classes = tdm.getTraningClassifications();//分类
        ClassifyMap.fileCount = tdm.getTraningClassifications().length;
        for (int i = 0; i < Classes.length; i++) {
            String Ci = Classes[i];//第i个分类
            //为第i个分类计算 先验条件  Ci文件数/总文件数
            ClassifyMap.priorProbability.put(Ci,PriorProbability.calculatePc(Ci));
            ClassifyMap.classFileCount.put(Ci, tdm.getTrainingFileCountOfClassification(Ci));
            Map<String, Integer> map = new HashMap<String, Integer>();
            ClassifyMap.classifyMap.put(Ci, map);
            //每个分类文件集合
            String[] filesPath = tdm.getFilesPath(Ci);
            //遍历每个分类
            for (String path:filesPath) {
                try {
                    String text = TrainingDataManager.getText(path);
                    String[] split = ChineseSpliter.split(text, " ").split(" ");//中文分词处理(分词后结果可能还包含有停用词）
        ...
        Map<String, Map<String, Integer>> classifyMap = ClassifyMap.classifyMap;
